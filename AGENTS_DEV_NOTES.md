...
- 世界模型要从简单计数器升级为“事件时间线 + 概念记忆”，能回答最近发生的事与概念出现情况。
- 自我模型需要清晰陈述“我会什么/不会什么”“最近在忙什么”，提供更高质量的自述。
- 驱动力应具备简单的目标队列与优先级，混合外部任务和内在动机。
- 工具层要有清晰协议，便于扩展到 HTTP / 文件 / 多模态工具（保持标准库实现的 stub）。
- Learning 模块要做规则级学习，例如记录工具成功率、意图到结果的统计。
- 对话策略需利用世界/自我信息，在回复中主动“自报家门”。
- Agent 主循环要更清晰、可观测，并预留多 Agent 交互的最小脚手架。
- 补日志、可视化与文档，方便调试和理解系统行为。
