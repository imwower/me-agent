...
class VideoEncoderStub:
    ...
    def encode(self, *args: Any, **kwargs: Any) -> List[float]:
        ...
        payload_str = f"video:{repr((payload_tuple, kwargs_items))}"

        logger.info("视频编码输入: args=%s, kwargs=%s", args, kwargs)
        vector = _hash_to_vector(payload_str)
        logger.info("视频编码结果向量长度=%d", len(vector))
        return vector
